...
| 🚧 | [BR-30](stories/BR-30-workers-horizon-scheduler.md) | Files, Horizon, planificateur | 5 | Les deux | T5, deux inscriptions à faire |
...
- **BR-30 T5 — alerte sur file non consommée.** Deux signaux sont livrés : le sondage `up/queue`, qui
  nomme l'état du worker, et `race:queue-heartbeat`, que le planificateur exécute chaque minute et qui
  ne ping que si la file est consommée — son silence couvre aussi le planificateur mort
  ([D-67](DECISIONS.md)). Restent deux inscriptions chez les services de surveillance, sans quoi
  personne n'écoute.
...
- **BR-30 T5 d'abord**, parce que c'est la seule qui coûte déjà. Les deux mails du parcours — le lien
  ...
  Le sondage `up/queue` et le battement `race:queue-heartbeat` sont livrés et éprouvés sur file mise
  en pause ; restent le moniteur et le check à inscrire chez les deux services ([D-67](DECISIONS.md)).
...
BR-30 T5 a livré son sondage `up/queue` et son battement `race:queue-heartbeat` ; sa case ne se ferme
qu'une fois les deux inscriptions faites chez les services de surveillance.
